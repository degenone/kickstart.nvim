...
function M.build_command(path)
  ...
  if os_name == "Windows_NT" then
    return string.format('powershell.exe -Command "Set-Clipboard -Path \'%s\'"', path)
...
function M.copy_to_clipboard(path, callback)
  if not path then
    ...
    if not path then
      ...
      if callback then callback(false, msg) else vim.notify(msg, vim.log.levels.ERROR) end
      return
    end
  end
  ...
  local cmd, err = M.build_command(path)
  if not cmd then
    if callback then callback(false, err) else vim.notify(err, vim.log.levels.ERROR) end
    return
  end
